...
def helper_face_point(shape, directive):
  ...
  if m:
    if len(m.groups())==2:
      ...
      raise_(Exception, 'Could not interpret directive: %s'%directive)
  else:
    try:
...
def root_curvature(w,side,dx,n=16):
  ...
  kappa = lambda t:  old_div(np.polyval( pn, t ),( np.polyval( pd, t )**(1.5))) # d Tangent angle/ds 
...
def mean_curvature(w,side,dx,n=16):
  ...
  kappa = lambda t:  old_div(dx*np.polyval( pn, t ),( np.polyval( pd, t )**(1.5))) # d Tangent angle/ds * ds/dt 
